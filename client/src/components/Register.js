...
import { registerHandler } from "../Api/apiHandler";
...
const Register = () => {
  ...
  return (
    <main className="p-main__contaiener register relative">
      {isLoading && (
        <div
          role="status"
      ...
      <div className="inner-container flex flex-col gap-10 items-center justify-center w-full h-screen mx-auto">
        <div className="header max-w-lg w-full flex flex-col gap-1 justify-start">
          ...
          <p className="text-primary text-lg">Register to get started</p>
        </div>
        <form
          onSubmit={handleSubmit}
          className="middle flex flex-col gap-2 max-w-lg w-1/2 items-center text-black"
        >
          <span className="border bg-grayInput rounded-lg w-full px-4 flex justify-between items-center gap-2">
            <label className="text-gray-500" htmlFor="name">
              <UserCircle />
            </label>
            <input
              className="focus:outline-none w-full bg-inherit py-2"
              type="text"
              required
              value={name}
          ...
          <span className="border bg-grayInput rounded-lg w-full px-4 flex justify-between items-center gap-2">
            ...
            />
            <span
              className="h-full cursor-pointer flex items-center text-gray-500"
              onClick={PasswordToggler}
            >
              {isPass ? <EyeDisable /> : <Eye />}
            </span>
          </span>
          <span className="rounded-lg w-full py-2 px-4 flex justify-between items-start gap-2">
            <input
          ...
          <button
            className="bg-primary w-full rounded-lg text-white border border-white py-2 hover:bg-white hover:text-primary hover:border-primary"
            type="submit"
          >
            Register
          </button>
      ...
      {status && (
        ...
        >
          ...
          <div>
            ...
            {status.message === "REGISTER_SUCCESS"
              ? ": A verification email has been sent to your email. Please verify your email and login"
              : status.message === "EMAIL_EXISTS"
              ? ": This Email has already been added. If you have an account please login"
